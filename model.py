...
class CustomBCELoss(nn.Module):
    ...
    def __init__(self, pos_weight: torch.Tensor, eps: float = 1e-6, reduction: str = "mean"):
        super().__init__()

        pos_weight = torch.as_tensor(pos_weight, dtype=torch.float32)
        if torch.isinf(pos_weight).any() or torch.isnan(pos_weight).any():
            raise ValueError("pos_weight contains inf/nan")
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.clamp(input, self.eps, 1. - self.eps)
        pw = self.pos_weight.to(dtype=x.dtype, device=x.device)
        while pw.dim() < x.dim():
            pw = pw.unsqueeze(-1)

        loss = -(pw * target * torch.log(x) + (1. - target) * torch.log(1. - x))

        if torch.isnan(loss).any():
            raise RuntimeError("CustomBCELoss produced NaN – check inputs!")
